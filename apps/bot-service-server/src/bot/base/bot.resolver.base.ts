...
export class BotResolverBase {
  ...
  async createBot(@graphql.Args() args: CreateBotArgs): Promise<Bot> {
    return await this.service.createBot({
      ...
      data: {
        ...
        baseBot: args.data.baseBot
          ? {
              connect: args.data.baseBot,
            }
          : undefined,
      },
    });
  ...
  async getBaseBot(@graphql.Parent() parent: Bot): Promise<BaseBot | null> {
    const result = await this.service.getBaseBot(parent.id);
